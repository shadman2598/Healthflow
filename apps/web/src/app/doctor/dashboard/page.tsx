  ...
  return (
    <ProtectedRolePage allowedRoles={["DOCTOR"]}>
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-slate-500">Your clinical overview</p>
      </div>
      ...
      {loading ? (
        ...
        <>
          <div className="grid gap-6 sm:grid-cols-3">
            <KpiCard title="Today's Appointments" value={appointments.length} icon={<IconCalendar className="h-6 w-6" />} iconBg="bg-teal-50 text-teal-600" />
            <KpiCard title="Assigned Messages" value={assignedThreads} icon={<IconChat className="h-6 w-6" />} />
            <KpiCard title="Unassigned Messages" value={unassignedThreads} icon={<IconClock className="h-6 w-6" />} iconBg="bg-amber-50 text-amber-600" />
          </div>
                  ...
                  {appointments.map((appt) => (
                    <div key={appt.id} className="flex items-center justify-between px-6 py-3.5">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">{formatTime(appt.scheduledAt)} · {appt.category.replace("_", " ")}</p>
                      </div>
                  ...
                  {threads.slice(0, 6).map((thread) => (
                    <div key={thread.id} className="px-6 py-3.5">
                      ...
                      <p className="text-xs text-slate-500">
                        {thread.patientProfile ? `${thread.patientProfile.firstName} ${thread.patientProfile.lastName}` : "Patient"} · {thread.status}
                      </p>
                    </div>
                  ))}
                </div>
